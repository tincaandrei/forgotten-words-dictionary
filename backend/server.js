...
if (!process.env.FAMILY_ACCESS_CODE) {
  console.warn('Warning: FAMILY_ACCESS_CODE is not set in environment variables.');
}
...
// Health check
app.get('/', (req, res) => {
  res.json({ message: 'Family dictionary backend is running.' });
});
...
app.post('/api/check-access', (req, res) => {
  ...
  return res
    .status(401)
    .json({ ok: false, error: 'Invalid access code' });
});
